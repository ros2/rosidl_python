...
def generate_py(generator_arguments_file, typesupport_impls):
    ...
    # expand templates which are common for all typesupport implementation
    for template_file, output_file in (
        ('_idl_pkg_bases.c.em', '_%s_bases.c' % package_name),
        ('_idl_pkg_import.c.em', '_%s_import.c' % package_name),
        ('_idl_pkg_decl.h.em', '_%s_decl.h' % package_name),
    ):
        template_file = os.path.join(template_dir, template_file)
        generated_base_file = os.path.join(args['output_dir'], output_file)
        data = {
            'package_name': args['package_name'],
            'content': idl_content,
        }
