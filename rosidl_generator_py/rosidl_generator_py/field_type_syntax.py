...
``get_fields_and_field_types(syntax='msg')`` without changing the stored
IDL representation.
"""
...
def idl_to_msg_syntax(idl_type: str) -> str:
    """Convert a single OMG IDL 4.2 field-type string to ROS 2 .msg syntax.

    :param idl_type: A field-type string as stored in a generated message
        class's ``_fields_and_field_types`` dict, e.g.
        ``sequence<string<256>, 32>``.
    :return: The equivalent ROS 2 .msg-syntax string, e.g.
        ``string<=256[<=32]``.
    """
    match = _BOUNDED_SEQUENCE_RE.match(idl_type)
    if match:
        value_type, max_size = match.groups()
        return f'{idl_to_msg_syntax(value_type)}[<={max_size}]'

    match = _UNBOUNDED_SEQUENCE_RE.match(idl_type)
    if match:
        value_type = match.group(1)
        return f'{idl_to_msg_syntax(value_type)}[]'

    return _BOUNDED_STRING_RE.sub(r'\1<=\2', idl_type)
